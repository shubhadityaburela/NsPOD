...
try:
    ...
    print(f"data is missing (Please contact the developer): {e}")
    sys.exit(1)
...
class ShapeShiftNet(nn.Module):
    def __init__(self, N_in, N_in_c, N_out, N_out_c, N_hidden, N_layers, x, t, center_matrix):
        ...
        # Hidden layers.
        self.hidden_layers_frame1 = torch.nn.Sequential(
            *[torch.nn.Sequential(*[torch.nn.Linear(N_hidden, N_hidden), activation()])
        ...
        # Hidden layers.
        self.hidden_layers_frame2 = torch.nn.Sequential(
            *[torch.nn.Sequential(*[torch.nn.Linear(N_hidden, N_hidden), activation()])
              for _ in range(N_layers - 1)])
        # Output layer.
        ...
        # Hidden layers.
        self.hidden_layers_frame3 = torch.nn.Sequential(
            *[torch.nn.Sequential(*[torch.nn.Linear(N_hidden, N_hidden), activation()])
              for _ in range(N_layers - 1)])
        # Output layer.
        ...
        # Hidden layers.
        self.hidden_layers1 = torch.nn.Sequential(
            *[torch.nn.Sequential(*[torch.nn.Linear(N_hidden, N_hidden), activation()])
              for _ in range(N_layers - 1)])
        # Output layer.
...
if pretrained_load:
    ...
    jit_model.eval()
    with torch.no_grad():
        f1_full, f2_full, f3_full, shift1_pred_raw, f1_full_nos, f2_full_nos, f3_full_nos = jit_model()

        # Isolating specific JIT views using .clone() to prevent structural exceptions on macOS/MPS
        T1Q1 = f1_full.view(Nx, Nt).clone()
        T2Q2 = f2_full.view(Nx, Nt).clone()
        T3Q3 = f3_full.view(Nx, Nt).clone()
        Q1 = f1_full_nos.view(Nx, Nt).clone()
        Q2 = f2_full_nos.view(Nx, Nt).clone()
        Q3 = f3_full_nos.view(Nx, Nt).clone()
        ...
        l1_loss = lambda_l1 * (torch.linalg.norm(T1Q1, 1) + torch.linalg.norm(T2Q2, 1) + torch.linalg.norm(T3Q3,
                                                                                                           1)) / torch.linalg.norm(
            Q, 1)
        nuclear_loss = lambda_star * (
                    NuclearNormAutograd.apply(Q1) + NuclearNormAutograd.apply(Q2) + NuclearNormAutograd.apply(Q3))
        total_loss = frobenius_loss + nuclear_loss + l1_loss

        print(
            f'Pretrained Model Evaluation -> F: {frobenius_loss.item():.4f}, N: {nuclear_loss.item():.4f}, L1: {l1_loss.item():.4f}, T: {total_loss.item():.4f}')
else:
    # --- TRAINING MODE ---
    jit_model = torch.jit.script(model)
    jit_model.to(device)
