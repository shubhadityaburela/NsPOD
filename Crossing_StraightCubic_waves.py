...
impath = "./data/Crossing_StraightCubic_waves/"
immpath = "./plots/Crossing_StraightCubic_waves/"
...
if pretrained_load:
    state_dict_original = torch.load("./trained_weights/Crossing_StraightCubic_waves/Crossing_StraightCubic_waves.pth", map_location=device)
    ...
    with torch.no_grad():
        ...
        # Isolating specific JIT views using .clone() to prevent low-level pointer exceptions on macOS/MPS
...
if not pretrained_load:
    torch.save(model.state_dict(), impath + 'Crossing_StraightCubic_waves.pth')
